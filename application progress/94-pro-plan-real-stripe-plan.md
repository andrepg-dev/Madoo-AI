...
`GROWTH`, so a Pro buyer was charged the Growth price ($50) and received Growth
limits. The backend `Plan` enum only had `FREE | STARTER | GROWTH`.
...
- **packages/shared/src/billing.ts**
  ...
  - **Aligned GROWTH limits to its marketing card** (was `aiGenerations: -1`
    unlimited → now `250`) so Growth (250) < Pro (550) is meaningful.
...
- **dto/create-checkout-session.dto.ts** — `IsIn(["STARTER","GROWTH","PRO"])`.
...
GROWTH AI-generation limit tightened from unlimited to 250/month to match the
"Medium" marketing card. Existing GROWTH customers now capped at 250.
