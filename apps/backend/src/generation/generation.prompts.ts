...
export const STATIC_INSTRUCTION = [
  ...
  "EMAIL ICON CATALOG: When compact icons would improve feature rows, benefits, contact details, trust cues, commerce details, or social links, call get_email_icons with 1-8 catalog names and dark/light tone. Two styles exist: style=outline (bare stroke glyphs, quiet and minimal) and style=badge (shaped icons — the glyph reversed out of a filled circle, bolder and more designed; pass color=<brand accent hex> so the badges match the palette). Pick ONE style per email and use it consistently: badge suits feature rows, benefit lists, and consumer/marketing briefs; outline suits minimal, editorial, or transactional emails. Display outline icons at 20-28px and badge icons at 28-36px as direct <Img src> constants; do not add icon URLs to variableSchema. Use icons sparingly. Never hand-build inline SVG, use icon fonts, or substitute emoji. Catalog icons do not satisfy the meaningful-image requirement.",
  ...
  "BACKGROUND-IMAGE HEADERS (use often): when the brief calls for a strong header/hero image — promos, launches, events, lifestyle brands, seasonal campaigns — PREFER building the header as a <Section> with an inline backgroundImage (style={{ backgroundImage: `url(${heroImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: '<fallback hex sampled from the image>' }}) and overlay the headline, eyebrow, and CTA directly on top of it, instead of stacking a plain <Img> above the text. This makes emails feel designed and unique. Rules: always set a backgroundColor fallback so text stays readable where background images are stripped (Outlook Windows ignores CSS background images — the fallback color must contrast with the overlaid text); keep overlaid text large and high-contrast, adding a subtle dark/light tint via a solid backgroundColor with transparency on an inner wrapper only when the photo needs it; give the section generous vertical padding (56-96px) so the image shows; bind the image URL to the same static image variable rules as any other image. Use a plain <Img> hero instead when the image itself is the content (product shots, screenshots, charts) or the email is transactional/minimal.",
