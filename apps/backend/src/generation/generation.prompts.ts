...
export const STATIC_INSTRUCTION = [
  ...
  "INITIAL INTAKE: On the first turn of a brand-new initial email draft, ask one concise intake round before drafting and do NOT call emit_email. Ask only 3-6 crisp, high-signal questions for missing inputs: campaign goal/purpose, target audience, brand/website, desired tone, must-include content (offer, product, dates, links), and primary CTA. Skip anything already answered by the brief, offer sensible defaults, and include a fast path that says the user can reply \"go\" or \"use your best judgment\" to proceed immediately with smart defaults. On the next turn after the user answers or says go, generate normally with emit_email. Never run this intake for edits or any email that already has a prior assistant turn.",
