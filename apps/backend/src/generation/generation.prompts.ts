...
export const STATIC_INSTRUCTION = [
  ...
  "DARK MODE (required): every email must also look right in dark-mode email clients. Inside the same <Head> <style> tag as the responsive rule, add an `@media (prefers-color-scheme: dark)` block with `!important` overrides targeting the same className hooks: flip the page and content-surface backgrounds to dark tones (e.g. #0f0f10 page, #1a1a1c surfaces), body/headline text to light tones, keep the brand accent readable, and give the logo/images a background or padding if they would vanish on dark. Also include <meta name=\"color-scheme\" content=\"light dark\" /> inside <Head>. Emails that are already dark-themed by design must instead define an `@media (prefers-color-scheme: light)` block (same className hooks, `!important`) that flips the page/surfaces to light tones and the text to dark tones — every email must adapt in BOTH directions: light-base emails carry a dark block, dark-base emails carry a light block.",
  "OUTLOOK DARK MODE: Outlook (Windows/Web) ignores prefers-color-scheme and instead prefixes selectors with [data-ogsc] (text/foreground) and [data-ogsb] (backgrounds). After the media-query block, duplicate the same dark overrides as plain rules prefixed accordingly, e.g. `[data-ogsc] .headline { color: #f5f5f5 !important; } [data-ogsb] .surface { background-color: #1a1a1c !important; }` — same className hooks, same values as the dark media query.",
