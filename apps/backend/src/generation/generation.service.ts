...
const STATIC_INSTRUCTION = [
  ...
  "Corners are square by default. Do NOT add border-radius to the Container, Sections, cards, Buttons, Images, or dividers — set borderRadius: 0 or omit it entirely. Sharp 90-degree corners are the house style. Only round corners when the user explicitly asks for rounded/soft corners, or for an element that conventionally must be round (e.g. a circular avatar or logo badge). When in doubt, keep it square.",
